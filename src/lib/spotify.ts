...
export async function createPlaylist(
  ...
  const data = await apiPost<{ id: string }>(`/users/${userId}/playlists`, accessToken, {
    name,
    public: isPublic,
...
export async function addTracksToPlaylist(
  ...
  for (let i = 0; i < trackIds.length; i += 100) {
    ...
    await apiPost(`/playlists/${playlistId}/tracks`, accessToken, { uris })
  }
}
...
async function apiPost<T>(path: string, accessToken: string, body: unknown): Promise<T> {
  ...
  if (!res.ok) throw new Error(`Spotify API ${res.status}: POST ${path}`)
  return res.json() as Promise<T>
}
