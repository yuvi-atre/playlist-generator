    ...
    >
      ...
      {FLOAT_NOTES.map((n, i) => (
        <svg
          ...
          style={{ left: n.left, width: n.size, height: n.size, color: n.color, animationDelay: n.delay }}
          fill="currentColor"
          aria-hidden="true"
        >
      ...
      <span className="relative text-xs tracking-wide text-zinc-500">now curated for your vibe</span>
    </div>
  )
}
...
const SUCCESS_SPARKS: { size: number; dx: number; dy: number; delay: string; color: string; note?: boolean }[] = [
    ...
    <div className={`relative w-[114px] h-[140px] ${className ?? ''}`}>
      {SUCCESS_SPARKS.map((d, i) => (
        <svg
          ...
          fill="currentColor"
        >
          {d.note ? <path d="M14 3v10.55A4 4 0 1 0 16 17V7h4V3h-6z" /> : <circle cx="12" cy="12" r="12" />}
        </svg>
      ))}
      ...
      <svg viewBox="0 0 240 300" className="mascot-sway absolute inset-0 w-full h-full overflow-visible" aria-hidden="true">
...
export function SuccessCheck({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 20 20" className={className ?? 'w-[18px] h-[18px]'} aria-hidden="true">
      <circle className="check-ring" cx="10" cy="10" r="8.5" fill="none" stroke="#1db954" strokeWidth="1.6" />
      <path
